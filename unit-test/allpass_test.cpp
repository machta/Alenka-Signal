...
void test(function<void(FilterProcessor<T>*)> change, function<void(T, T)> compare)
{
...

	{
		...
		cl_mem inBuffer = clCreateBuffer(context.getCLContext(), flags | CL_MEM_COPY_HOST_PTR, n*sizeof(T), signal.data(), &err);
		checkClErrorCode(err, "clCreateBuffer");

		cl_mem outBuffer = clCreateBuffer(context.getCLContext(), flags, n*sizeof(T), nullptr, &err);
		checkClErrorCode(err, "clCreateBuffer");
